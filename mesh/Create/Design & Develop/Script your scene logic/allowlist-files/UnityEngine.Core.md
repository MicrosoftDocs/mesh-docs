...
|`localToWorldMatrix`|UnityEngine\.Matrix4x4|Matrix that transforms a point from local space into world space \(Read Only\)\.|yes|no|no|Transform \| Get Local To World Matrix
|`lossyScale`|UnityEngine\.Vector3|The global scale of the object \(Read Only\)\.|yes|no|no|Transform \| Get Lossy Scale
...
|`worldToLocalMatrix`|UnityEngine\.Matrix4x4|Matrix that transforms a point from world space into local space \(Read Only\)\.|yes|no|no|Transform \| Get World To Local Matrix
...
|`fixedDeltaTime`|float|The interval in seconds at which physics and other fixed frame rate updates \(like MonoBehaviour's MonoBehaviour\.FixedUpdate\) are performed\.|yes|yes|Time \| Get Fixed Delta Time<br>Time \| Set Fixed Delta Time
