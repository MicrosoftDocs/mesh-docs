...
## Setting up a scene with Mesh Visual Scripting

- In Unity, on the **Mesh Toolkit** menu, select **Configure Visual Scripting**.

   This ensures that Mesh Visual Scripting is configured to show you the node options you can use with Mesh. From there, it's basically just Unity Visual Scripting with a few extensions. 
...
**_Current limitation:_** Mesh Uploader doesn't currently re-validate visual scripts before uploading them. If any visual script flows contain errors (shown in the diagnostics panel), they'll upload without error, but they won't be executed in Mesh.

----------------------------------------------------------------------------------------------------

## Sharing and networking

### Shared and local scene state
...
- User-controlled local state – sub-scenes marked with the **Local Scene Scope** component.
...
![Screen shots of the On State Changed event node connecting to the result of comparing a Get Variable node to an integer literal.](../../../media/mesh-scripting/visual-scripting//node-OnStateChanged-volatile.png)
...
- Do use **Local Scene Scope** components liberally to make sure only variables and component properties that _need_ to be synchronized across clients incur networking overhead. Anything animated locally with a visual script should not be shared.
...
- Don't update shared properties at high frequency. Instead, consider making high-frequency updated properties local by using the **Local Scene Scope** component. Remember that visual script variables are also shared by default!
...
Consider a scene that uses an **On Update** trigger on 10 balloons, updating their _shared_ position each frame on 10 clients: 10 balloons x 10 clients x 60 frames/second result in 6000 messages per second sent over the network. If each message requires 200 bytes (including networking overhead), this causes additional network traffic of roughly 1 Mbit/second.

To fix a situation like this, consider the following options:

- Place a **Local Scene Scope** component above the balloons in the transform hierarchy to avoid network replication.
